...
void CreateSketchBigCurvedPipe(RPArray<MbContour>& _arrContours, double ttDiam, double ttThickness, double t)
{
    // Построение образующей в виде двух концентрических окружностей
    const double RAD_EXT = ttDiam / 2; // Радиус внешней окружности
    const double RAD_INT = (ttDiam - 2 * ttThickness) / 2; // Радиус внутренней окружности
...
SPtr<MbSolid> ParametricModelCreator::createBigCurvedPipe_011(double ttDiam, double ttThickness, double assemblyHeight, double t)
{
    ...
    RPArray<MbContour> arrContours;
    CreateSketchBigCurvedPipe(arrContours, ttDiam, ttThickness, length);
